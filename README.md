...
- кастомные хуки useFetch и useFetchCached для взаимодействия с сервером, обработки данных и статусов (data, isLoading, isError)
...
- Каждый четвертый счетчик на странице отрисовывется как компонент (CounterSpecial) и не содержит кнопок "+" и "-" , а каждую секунду увеличивает свое значение на единицу
- Для компонента обычного счетчика (Counter) применена оптимизация React.memo, чтобы при изменении родительского элемента со списком счетчиков (CounterList) компоненты Counter не перерисовывались при отсутствии изменений
